    ...
    for (size_t i = 0; i < input_rows_count; ++i)
    {
        ...
        if (separator.empty())
        {
            for (size_t j = 0; j < haystack.size(); ++j)
                res_data.push_back(haystack[haystack.size() - 1 - j]);
            res_offsets[i] = res_data.size();
            continue;
        }
        ...
        if (separator.size() == 1)
        {
            const char sep_char = separator[0];
            ...
            while (pos < haystack.size())
            {
                ...
                /// Find next separator
                size_t next_pos = pos;
                while (next_pos < haystack.size() && haystack[next_pos] != sep_char)
                    ++next_pos;
...
REGISTER_FUNCTION(ReverseBySeparator)
{
    ...
    // Insert empty parameters to match current FunctionDocumentation layout (Parameters is a vector<Argument>).
