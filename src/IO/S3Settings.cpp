...
{
    auth_settings = S3AuthSettings(config, settings, config_prefix);
    request_settings = S3RequestSettings(config, settings, config_prefix);
}
    ...
    const DB::Settings & settings)
{
    ...
    auto default_auth_settings = S3AuthSettings(config, settings, config_prefix);
    auto default_request_settings = S3RequestSettings(config, settings, config_prefix);
    ...
    for (const String & key : config_keys)
    {
        ...
        if (config.has(endpoint_path))
        {
            ...
            auth_settings.updateIfChanged(S3AuthSettings(config, settings, key_path));

            auto request_settings{default_request_settings};
            request_settings.updateIfChanged(S3RequestSettings(config, settings, key_path, "", settings[Setting::s3_validate_request_settings]));
